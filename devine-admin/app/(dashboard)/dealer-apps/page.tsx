...
export default function DealerAppsPage() {
  ...
  const fetchLeads = useCallback(async () => {
    setLoading(true)
    const params = new URLSearchParams({ source: 'dealer-form' })
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-[#1B2B4B]">Dealer applications</h1>
        <p className="text-sm text-gray-500 mt-1">Submitted from the become-a-dealer page</p>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="hidden lg:table-cell">Products wanted</TableHead>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={11} className="text-center py-12 text-gray-400">No dealer applications yet</TableCell></TableRow>
              ) : leads.map((lead, i) => (
                <TableRow key={lead._id}>
                  <TableCell className="text-gray-400 text-sm">{i + 1}</TableCell>
                  <TableCell className="font-medium whitespace-nowrap">{lead.businessName ?? <span className="text-gray-300">—</span>}</TableCell>
                  ...
                  <TableCell className="hidden lg:table-cell text-sm text-gray-600 max-w-[200px]">
                    {lead.products
                      ? lead.products.split(', ').map((p: string) => (
                          <span key={p} className="inline-block bg-gray-100 text-gray-700 text-xs px-2 py-0.5 rounded mr-1 mb-0.5">{p}</span>
                        ))
